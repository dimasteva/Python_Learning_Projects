...
class GameUI:
    def __init__(self, window):
        ...
        self.max_score = 0  # Initialize the maximum score
        self.create_grid()
        self.start_new_game()
    ...
    def get_cell_color(self, number):
        ...
        return colors.get(number, '#3c3a32')  # Default color for numbers greater than 2048
    ...
    def handle_key_press(self, event):
        ...
        if not self.game.generate_random_cell():
            score = self.game.return_score()
            if score > self.max_score:
                self.max_score = score  # Update max score if needed
